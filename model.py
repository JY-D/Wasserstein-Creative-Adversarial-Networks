...
class DCGAN(object):
  ...
  def build_model(self, old_model=False):
    ...
    if self.can:
      ...
      self.g_loss = -self.d_loss_fake + self.lamb * self.g_loss_class_fake    
  ...
  def test(self, config):
    ...
    batch_z_result = self.sess.run([self.G],
            feed_dict={
              self.z: batch_z,
            })[0]
    np.save('batch_z_result.npy', batch_z_result)
